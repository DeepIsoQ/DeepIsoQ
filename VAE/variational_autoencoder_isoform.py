...
# Add a new module to predict isoform expression from the latent space
class IsoformRegressor(nn.Module):
...
mse_iso = nn.MSELoss(reduction="mean")   # supervised loss on Y
lambda_iso = 1e-3  # weight for the isoform loss (you can tune this)
...
optimizer = torch.optim.Adam(
    list(vae.parameters()) + list(iso_head.parameters()), 
    lr=3e-5
)
max_grad_norm = 1.0
...
while epoch < num_epochs:
    ...
    for x, y in train_loader:
        ...
        # --- Unsupervised VAE loss (ELBO) ---
        # perform a forward pass through the model and compute the ELBO
        unsup_loss, diagnostics, outputs = vi(vae, x)
        z = outputs['z'] # latent representation (batch_size, latent_features)
        
        # --- Supervised Isoform prediction loss ---
        y_pred = iso_head(z)  # (batch_size, n_isoforms)
        sup_loss = mse_iso(y_pred, y) # MSE on log1p(Y_tx)

        # --- Total loss ---
        total_loss = unsup_loss + lambda_iso * sup_loss

        optimizer.zero_grad()
        total_loss.backward()
        # gradient clipping
        torch.nn.utils.clip_grad_norm_(vae.parameters(), max_grad_norm)
        torch.nn.utils.clip_grad_norm_(iso_head.parameters(), max_grad_norm)
        optimizer.step()

        # gather data for the current bach
        for k, v in diagnostics.items():
            training_epoch_data[k].append(v.mean().item())
        training_epoch_data["iso_loss"].append(sup_loss.item())
        training_epoch_data["total_loss"].append(total_loss.item())


    # gather data for the full epoch
    for k, v in training_epoch_data.items():
        training_data[k].append(np.mean(v))
    ...
    with torch.no_grad():
        ...
        val_unsup_loss, val_diagnostics, val_outputs = vi(vae, x_val)
        z_val = val_outputs["z"]
        y_val_hat = iso_head(z_val)
        val_iso_loss = mse_iso(y_val_hat, y_val)
        val_total_loss = val_unsup_loss + lambda_iso * val_iso_loss

        for k, v in val_diagnostics.items():
            validation_data[k].append(v.mean().item())
        validation_data["iso_loss"].append(val_iso_loss.item())
        validation_data["total_loss"].append(val_total_loss.item())
    ...
    print(f"Epoch {epoch:03d} | loss={total_loss.item():.3f} | elbo={diagnostics['elbo'].mean().item():.3f} | kl={diagnostics['kl'].mean().item():.3f}")
