#!/usr/bin/env python3
"""
FFNN Random Search on BulkFormer Embeddings (GPU + AMP)

This script trains feed-forward neural networks to predict isoform 
expression from **BulkFormer transcriptome-level embeddings**.

Differences to the gene-expression version:
- Input X = X_bulkformer_emb (N, D_embed) from bulkformer_result.pt
- Y = log1p(Y_tx) as before, to keep targets comparable
...
What is saved:
- arch_search_results_bulkformer.csv  → all trial metrics
- arch_search_summary_bulkformer.json → best model + top 5 summary
- best_model_bulkformer.pt            → best model weights + preprocessing stats
- figs_trials_bulkformer/curves_*.png → train/val curves for each trial
- summary_val_mse_bar_bulkformer.png  → barplot of validation MSEs
- top5_val_curves_bulkformer.png      → top-5 validation curves
"""
...
RESULTS_DIR         = "ffnn_search_bulkformer/results"
RESULTS_CSV         = os.path.join(RESULTS_DIR, "arch_search_results_bulkformer.csv")
BEST_MODEL_PT       = os.path.join(RESULTS_DIR, "best_model_bulkformer.pt")
SUMMARY_JSON        = os.path.join(RESULTS_DIR, "arch_search_summary_bulkformer.json")

os.environ.setdefault("MPLBACKEND", "Agg")
TRIAL_FIG_DIR       = os.path.join(RESULTS_DIR, "figs_trials_bulkformer")
SUMMARY_FIG_BAR     = os.path.join(RESULTS_DIR, "summary_val_mse_bar_bulkformer.png")
SUMMARY_FIG_TOP5    = os.path.join(RESULTS_DIR, "top5_val_curves_bulkformer.png")
BEST_FIG_CURVES     = os.path.join(RESULTS_DIR, "best_model_curves_bulkformer.png")
...
print(f"[INFO] Loading BulkFormer embeddings from: {BF_PT}")
...
# Y: log1p of isoform expression (same transform as gene-based script)
Y = torch.log1p(data["Y_tx"].float()).cpu().numpy()  # (N, I)

N, D_embed = X.shape
_, I = Y.shape
print(f"[INFO] Shapes: X_bulkformer_emb={X.shape}, Y_tx(log1p)={Y.shape}")
print(f"[INFO] N={N}, D_embed={D_embed}, I={I}")
    ...
    for epoch in range(1, hp["epochs"] + 1):
        ...
        if epoch == 1 or epoch % EVAL_EVERY == 0:
            ...
            print(f"[{hp['name']}] ep {epoch:03d} | act={hp['act']} | "
                  f"train {epoch_train:.5f} | val {val_mse:.5f} | lr {opt.param_groups[0]['lr']:.4g}")
            if noimp >= PATIENCE:
...
# Run search
# ------------------------------
print(f"[INFO] Starting random search on BulkFormer embeddings with {N_TRIALS} trials...")
...
for t in range(N_TRIALS):
    ...
    if best_rec is None or rec["val_mse"] < best_rec["val_mse"]:
        ...
        torch.save({
            ...
            "meta": {"D_embed": D_embed, "I": I, "seed": SEED}
        }, BEST_MODEL_PT)
...
print(f"[BEST on TEST] BulkFormer FFNN | act={best_hp['act']} | "
...
print("\n=== BULKFORMER FFNN SEARCH COMPLETE ===")
...
try:
    ...
    plt.title("Validation MSE by model (BulkFormer embeddings)")
    plt.tight_layout()
...
try:
    ...
    plt.title("Top-5 models — validation curves (BulkFormer embeddings)")
...
print("[INFO] All done (BulkFormer FFNN).")
